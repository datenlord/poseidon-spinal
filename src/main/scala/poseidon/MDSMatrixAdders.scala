...
class MDSMatrixAdders(g:PoseidonGenerics) extends Component{
  ...
  def ModAdderVec(op1s:Vec[UInt],op2s:Vec[UInt],result:Vec[UInt]) = new Area{
  ...
  val threadAdders = new Area{
    ...
    val modAdderVec1 = ModAdderVec(inputs(0).state_elements,inputs(1).state_elements,tempRes.state_elements)
    ...
    val modAdderVec2 = ModAdderVec(tempOp1s.state_elements,tempOp2s,output.state_elements)
  }
  ...
  val threadAccumulator = new Area{
    ...
    val modAdderVec = ModAdderVec(tempRes.state_elements,input.state_elements,modAdderRes)
...
//     import AccumulatorState._
//     val tempRes = Reg(MDSContext(g)) init(MDSContextInitValue(g))
//     val accumulatorState = Reg(AccumulatorState()) init(IDLE)
//     val counter = Reg(UInt(log2Up(g.t_max) bits)) init(0)
...
//           tempRes.state_elements.assignFrom(input.state_elements)
//           tempRes.state_id := input.state_id
//           tempRes.state_size := input.state_size
//           tempRes.round_index := input.round_index

//           when(input.state_size === 3){
//             accumulatorState := DONE
//           } otherwise{
//             accumulatorState := ADDING
//             counter := counter + 3
//           }
//         }
//       }
