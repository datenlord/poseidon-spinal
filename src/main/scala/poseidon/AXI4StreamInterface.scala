...
class AXI4StreamReceiver(g:PoseidonGenerics) extends Component{
  ...
  val DataBuffer = new Area{
    ...
    val continue = DataController.receiverState===DONE|DataController.receiverState===BLOCK|DataController.receiverState===IDLE
    
    val buffer0 = DataController.output.stage()
    val buffer0_buffer1 = Stream(ReceiverContext(g))
    buffer0_buffer1.arbitrationFrom(buffer0.continueWhen(continue|buffer0.state_id=/=DataController.idCounter))
    ...
    buffer1_buffer2.arbitrationFrom(buffer1.continueWhen(continue|buffer1.state_id=/=DataController.idCounter))
    ...
    buffer2_buffer3.arbitrationFrom(buffer2.continueWhen(continue|buffer2.state_id=/=DataController.idCounter))
...
class AXI4StreamTransmitter(g:PoseidonGenerics, buffer_depth:Int) extends Component{
  ...
  (input_demux,StreamDemux(io.input, demux_select, buffer_depth)).zipped.foreach(_<<_)
  
  val buffer = input_demux.map(_.stage())
  val select = OHToUInt((buffer.map(_.valid),buffer.map(_.state_id)).zipped.map(_ & _===idCounter))
...
object AXI4StreamReceiverVerilog{
    ...
    def main(args: Array[String]): Unit = {
        val config = PoseidonGenerics(t_max = 12, round_max = 65, thread_num = 3, data_width = 255, id_width=5)
        SpinalConfig(
            mode=Verilog,
