...
class PoseidonThread(g:PoseidonGenerics) extends Component{
  ...
  val SBox5Stage = new Area {
    ...
    val select_fifo = fifo_input.translateWith(DemuxSelect).queueLowLatency(size = 20)
    val mux_output = StreamMux(select_fifo.payload,SBox5Insts.map(_.io.output))
    val output = StreamJoin.arg(mux_output,select_fifo).translateWith(mux_output.payload)
  }
  ...
  val MDSMixStage = new Area{
    ...
    val select_fifo = fifo_input.translateWith(DemuxSelect).queueLowLatency(size = 9)
    val mux_output = StreamMux(select_fifo.payload, matrixMultiplierInsts.map(_.io.output))
    val output = StreamJoin.arg(mux_output,select_fifo).translateWith(mux_output.payload)
  }
...
object PoseidonThreadVerilog{
  def main(args: Array[String]): Unit = {
    val config = PoseidonGenerics(t_max = 12,round_max = 65,thread_num = 3,data_width = 255,id_width=6)
    SpinalConfig(
      mode = Verilog,
