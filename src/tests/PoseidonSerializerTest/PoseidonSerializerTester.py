...
class PoseidonSerializerTester:
    ...
    async def drive_input_ports(self):
        ...
        while self.cases_count < CASES_NUM:
            ...
            for i in range(self.buffer_size):
                exec("dut.io_parallelInput_payload_state_elements_{}.value = state_elements[{}]".format(i,i))

            await RisingEdge(dut.clk)
            if (dut.io_parallelInput_valid.value & dut.io_parallelInput_ready.value) == True:
                self.cases_count += 1
                for i in range(state_size):
                    self.ref_outputs.put([round_index,i,state_size, state_id, state_elements[i]])
                if(state_size==5):
    ...
    async def check_output_ports(self):
        ...
        while True:
            ...
            if dut.io_serialOutput_ready.value & dut.io_serialOutput_valid.value == True:
...
def test_PoseidonSerializer():
    simulator.run(
        verilog_sources = ["./src/main/verilog/PoseidonSerializer.v"],
