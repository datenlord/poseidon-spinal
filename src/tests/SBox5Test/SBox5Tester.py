...
async def generate_input(dut):
    ...
    while cases_count < cases_num:
        ...
        if(dut.io_input_valid.value & dut.io_input_ready.value) == True:
            cases_count += 1
            ref_outputs.put([state_element.value, state_element.ModExp(state_element.value,5)])
...
async def output_check(dut):
    ...
    while cases_count < cases_num:
        ...
        if(dut.io_output_ready.value & dut.io_output_valid.value == True):
            ...
            assert dut_res == ref_res,"case num {}:the result of S-box({}) is wrong\nref:{}\ndut:{}".format(count_cases,hex(ref_input),hex(ref_res),hex(dut_res))
