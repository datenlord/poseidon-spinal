...
def get_random_input():
    ...
    for i in range(rand_size):
        rand_vec = []
        for j in range(12):
            if j < rand_size: rand_vec.append(random.randint(0,basic.p-1))
            else: rand_vec.append(0)
        rand_matrix.append(rand_vec)

    if rand_size == 5: rand_matrix.append(12*[0])
...
async def generate_input(dut):
    ...
    while cases_count < cases_num:
        ...
        if valid == False: await RisingEdge(dut.clk)

        if(valid == True):
            cases_count += 1
            for i in range(ceil(size/3)):
                set_input_ports(dut, matrix[i*3+0], matrix[i*3+1], matrix[i*3+2])
                await RisingEdge(dut.clk)
                while (dut.io_inputs_0_valid.value & dut.io_inputs_0_ready.value == False):
                    await RisingEdge(dut.clk)
...
async def output_check(dut):
    ...
    while cases_count < cases_num:
        ...
        if (dut.io_output_ready.value & dut.io_output_valid.value) == True:
            ...
            if not passed: 
                print("failed info: case_num:{} input size:{}".format(cases_count,size))
                print("ref output:")
